...
- Produces: `createRateLimiter(opts: { limit: number; windowMs: number; keyFor?: (req: Request) => string; maxEntries?: number }): RequestHandler`

A fixed-window counter in process memory, deliberately dependency-free — and deliberately not backed
by the shared `CacheStore`: its contract (`get`/`set`/`del`/`getdel`) has no atomic increment, so a
cache-backed counter built from get-then-set would race under concurrent requests for the same key,
silently undercounting and letting more requests through than configured. `/register` is
...
instance count. That is acceptable, well-understood shape for a spam brake and is documented rather
than engineered around — on the `registerRateLimit` field of `ShopifyMcpOAuthConfig` (Task 3's
`src/config.ts`) and again on the comment above `createRateLimiter` itself.

`maxEntries` (default 10,000) bounds worst-case memory: once the map holding one window per tracked
key is full, inserting a never-seen key evicts the single oldest tracked key rather than growing
further.
...
Expected: PASS, 5 tests. If a step fails, the mismatch is between two units that each pass in
