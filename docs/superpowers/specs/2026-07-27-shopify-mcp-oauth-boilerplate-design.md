...
const oauth = createShopifyMcpOAuth({
  ...
  storage: prismaStorage(prisma, { shopModel: "shop", domainField: "domain" }),
...
| Adapter | Use |
|---|---|
| `prismaStorage(prisma, opts)` | Prisma. `opts` maps the app's existing shop model/field, since no two apps name it the same. |
...
examples/basic-server/
  src/
    ...
    storage.ts           prisma or memory, per the CLI's choice
...
`Shop` exists here only so the demo has something to look up. A real adopter points
`prismaStorage`'s `shopModel` / `domainField` options at their own table.

### 4.4 Seeding

`pnpm db:seed` inserts one shop from `DEMO_SHOP_DOMAIN`. This must be the development store the
tester actually picks in Shopify's shop picker — otherwise the callback correctly returns
`shop_not_installed` and the first-run experience looks like a bug. The example README leads with
this.
...
- On **in-memory**, removes `prisma/`, `docker-compose.yml`, and the db scripts, and swaps
  `storage.ts` for the memory variant. Prints a warning that tokens are lost on restart and that this
